...
schoex.controller('mainController', function(dataFactory,$rootScope,$route,$scope) {
  ...
  angular.forEach($rootScope.dashboardData.academicYear, function (item) {
    if(item.id == $rootScope.dashboardData.selectedAcYear){
        //return item.yearTitle;
        $scope.currentAcademicYear = item;
    }
  });

  /*$scope.currentAcademicY = function() {
      angular.forEach($rootScope.dashboardData.academicYear, function (item) {
          if(item.id == $rootScope.dashboardData.selectedAcYear){
              //return item.yearTitle;
              $scope.currentAcademicYear = item;
...
schoex.controller('studentsController', function(dataFactory,CSRF_TOKEN,$rootScope,$scope,$sce,$route) {
  ...
  $scope.views.list = true;
  $scope.form = {};
  $scope.form.mobileNo = "+2348108013285";
  $scope.CSRF_TOKEN = CSRF_TOKEN;
  $scope.userRole ;
  $scope.importType ;
  $scope.importReview;
  ...
  $scope.changeView = function(view){
    if(view == "add" || view == "list" || view == "show"){
      ...
      {
        $scope.form.mobileNo = "+2348108013285";
        $scope.form.phoneNo = JSON.parse("08108013285");
      }*/
    }
    $scope.views.list = false;
    $scope.views.approval = false;
...
schoex.controller('examsListController', function(dataFactory,$rootScope,$scope,$sce,$filter) {
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('examsList/'+id).then(function(data) {
      ...
      $scope.form.closingDate = $filter('date')($scope.form.closingDate * 1000, $rootScope.angDateFormat);
      $scope.form.resumptionDate = $filter('date')($scope.form.resumptionDate * 1000, $rootScope.angDateFormat);
      showHideLoad(true);
    });
  }
