...
"classes"=>array("title"=>"classes","url"=>URL::to('#/classes'),"icon"=>"fa fa-sitemap","activated"=>"attendanceAct","cusPerm"=>"classes","permissions"=>array('admin'),
/*"children"=>array(
...
"terms"=>array("title"=>"schoolTerms","url"=>URL::to('#/terms'),"icon"=>"fa fa-file-text-o","cusPerm"=>"generalSettings","permissions"=>array('admin') ),
		...
		$this->settingsArray = $this->settingsArrayPrep($settings);
	...
	public function viewop($layout,$view,&$data,$div=""){
		if(Input::isAjax()){
			if($div != ""){
				echo "DBArea('".htmlspecialchars($this->sanitize_output( View::make($view, $data,true) ),ENT_QUOTES)."','".$div."');";
			}else{
				echo "DBArea('".htmlspecialchars($this->sanitize_output( View::make($view, $data,true) ),ENT_QUOTES)."');";
			}
			exit;
		}else{
			$data['content'] = View::make($view, $data,true);
			//$layout->with($data);
	...
	//hepler method
	protected function settingsArrayPrep($settings)
	{
		$setingsArray=[];
